...
| データ更新 | TypeScript (`lib/fetch-news.ts`), Upstash QStash | RSS → 翻訳 → KV 書込 |
...
cron を 1日4回実行し KV を更新。詳細は [SCHEDULER.md](./SCHEDULER.md)（Upstash QStash）。
...
|------|------|
| Footer 各ページ | リンクのみ（`#`） |
| Vercel Cron によるバッチ | 未採用（Upstash QStash を使用） |
| On-Demand Revalidation | 実装済（`/api/cron/fetch` 内 + `/api/revalidate`） |
| ユーザー認証 | なし |
| 記事本文表示 | 意図的に非対応 |
